<template>
  <div>
    ...
    <button style="display: none" @click="openLikesModal" ref="likeCount" :id="`likes-count-${feed.id}`">
      {{ feed.likes }}</button
    >
    <button
      ref="openLikesModal"
...
<script>
export default {
  props: ["feedId", "userId", "likes", "feed", "bus"],
  ...
  methods: {
    ...
    likeFeed() {
      ...
      axios
        .post("/posts/" + this.feed.id + "/" + this.userId + "/action", {
          action: action,
        })
        .then((response) => {
